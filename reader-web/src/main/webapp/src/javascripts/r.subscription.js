  ...
  r.util.ajax({
    ...
    type: 'GET',
      ...
      if ($(data.categories[0].categories).size() > 0 || $(data.categories[0].subscriptions).size() > 0) {
        // Building HTML tree
        var html = '<ul id="category-root" data-category-id="' + data.categories[0].id + '" ' + 'category-depth=0>';
...
/**
 * Building category li recursively with a maximum depth of 5.
 */
r.subscription.buildCategoryItem = function (category, depth = 1) {
  if (depth > 5) {
    return ''; // Stop recursion if depth exceeds 5
  }
  ...
  if (category.categories && category.categories.length > 0) {
    $(category.categories).each(function (i, subcategory) {
      subscriptionsHtml += r.subscription.buildCategoryItem(subcategory, depth + 1);
    });
  }
    
  subscriptionsHtml += '</ul>';
    
  return '<li id="category-' + category.id + '" data-category-id="' + category.id + '" ' + ' category-depth=' + depth + ' ' +
  ...
  $('#subscription-list ul').sortable({
    ...
    stop: function (event, ui) {
      ...
      if (ui.item.hasClass('subscription')) {
        ...
        // If the user drops a category not in the root category or exceeds depth limit, cancel and warn
        var parentDepth = parseInt(ui.item.parent().parent().attr('category-depth'));
        if (ui.item.parent().attr('id') != 'category-root' && parentDepth+1 > 5) {
        ...
        var parentCategoryId = ui.item.parent().parent().attr('data-category-id') || rootCategoryId;
        var order = ui.item.index();

        // Calling API to update category order and parent
        r.util.ajax({
          url: r.util.url.category_update.replace('{id}', categoryId),
          data: { order: order, parent: parentCategoryId },
          type: 'POST',
          fail: function (jqxhr) {
