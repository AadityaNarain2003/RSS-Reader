...
import static com.sismics.reader.core.constant.Constants.*;
...
public class SubscriptionImportAsyncListener {
    ...
    /**
     * Read the file to import in a 1st pass to know the number of feeds / starred
     * articles to import
    ...
    private Job createJob(final User user, File importFile) {
        ...
        try {
            ...
            if (MimeType.APPLICATION_ZIP.equals(mimeType)) {
                outlineCount = getOutlineCount(importFile, closer, outlineCount);
            } else {
            ...
            Job job = new Job(user.getId(), JOB_IMPORT.getValue());
            job.setStartDate(new Date());
            jobDao.create(job);

            JobEventDao jobEventDao = new JobEventDao();

            JobEvent jobEvent = new JobEvent(job.getId(), JOB_EVENT_FEED_COUNT.getValue(),
                    String.valueOf(outlineCount));
            jobEventDao.create(jobEvent);

            jobEvent = new JobEvent(job.getId(), JOB_EVENT_STARRED_ARTICLED_COUNT.getValue(),
    ...
    private long getOutlineCount(File importFile, Closer closer, long outlineCount) throws Exception {
        // Assume the file is a Google Takeout ZIP archive
        ZipArchiveInputStream archiveInputStream = null;
        archiveInputStream = closer
                .register(new ZipArchiveInputStream(new FileInputStream(importFile), Charsets.ISO_8859_1.name()));
    ...
    private void processImportFile(final User user, File importFile, final Job job) {
        ...
        try {
            ...
            String mimeType = MimeTypeUtil.guessMimeType(importFile);
            if (MimeType.APPLICATION_ZIP.equals(mimeType)) {
                outlineList = getOutlineList(importFile, closer, outlineList);
            } else {
                // Assume the file is an OPML file
                InputStream is = closer.register(new FileInputStream(importFile));
                OpmlReader opmlReader = new OpmlReader();
    ...
    private static List<Outline> getOutlineList(File importFile, Closer closer, List<Outline> outlineList) throws Exception {
        // Assume the file is a Google Takeout ZIP archive
        ZipArchiveInputStream archiveInputStream = null;
        archiveInputStream = closer.register(
                new ZipArchiveInputStream(new FileInputStream(importFile), Charsets.ISO_8859_1.name()));
        ArchiveEntry archiveEntry = archiveInputStream.getNextEntry();
        while (archiveEntry != null) {
            File outputFile = null;
            try {
        ...
        for (Entry<String, List<Outline>> entry : outlineMap.entrySet()) {
            ...
            for (int j = 0; j < categoryOutlineList.size(); j++) {
                ...
                if (!feedSubscriptionList.isEmpty()) {
                    ...
                    JobEvent jobEvent = new JobEvent(job.getId(), JOB_EVENT_FEED_IMPORT_SUCCESS.getValue(),
                            feedSubscriptionList.iterator().next().getFeedRssUrl());
                    jobEventDao.create(jobEvent);
                ...
                try {
                    ...
                    JobEvent jobEvent = new JobEvent(job.getId(), JOB_EVENT_FEED_IMPORT_FAILURE.getValue(), feedUrl);
                    jobEventDao.create(jobEvent);
                    continue;
                }
                ...
                try {
                    ...
                    JobEvent jobEvent = new JobEvent(job.getId(), JOB_EVENT_FEED_IMPORT_SUCCESS.getValue(), feedUrl);
                    jobEventDao.create(jobEvent);
                } catch (Exception e) {
                    if (log.isErrorEnabled()) {
...
//     * If some feed cannot be downloaded, a record is still created from the export
//     * data only.
//     *
//     * @param user User
...
//                            "Error importing the feed at URL {0} for user {1}''s stared articles. Maybe it doens't exist anymore?",
//                            rssUrl, user.getId()), e);
//                }
//                feedFromDb = new Feed();
...
//            if (log.isInfoEnabled()) {
//                log.info(MessageFormat.format("Cannot import starred article with an empty title and url for feed {0}",
//                        rssUrl));
//            }
//        }
