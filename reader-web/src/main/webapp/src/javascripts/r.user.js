...
r.user.registerFormHandler = function (panel, initialize) {
  console.log("IN FUNCTION");
  var form = $("#register-form");
  var usernameInput = form.find("#register-username");
  var emailInput = form.find("#register-email");
  var passwordInput = form.find("#register-password");
  var password2Input = form.find("#register-password2");
  initialize=true;
  if (initialize) {
    ...
    var validate = form.validate({
      rules: {
        ...
        password2: {
          required: true,
          equalTo: "#" + form.attr("id") + " #register-password",
        },
      },
      submitHandler: function () {
        ...
        r.util.ajax({
          ...
          data: {
            ...
            $().toastmessage("showSuccessToast", $.t("register.success"));
            form[0].reset();
          },
        });
...
r.user.switchToRegister = function () {
  ...
  var showJobs = function (userInfo) {
    ...
    if (inProgress) {
      ...
      $(userInfo.jobs).each(function (i, job) {
        if (
          job.feed_success + job.feed_failure != job.feed_total ||
          job.starred_success + job.starred_failure != job.starred_total
        ) {
          done = false;
        }
      });
      var html =
        "<p>" +
        (done ? $.t("jobs.done") : $.t("jobs.inprogress")) +
        (done
          ? ' <a href="#" class="dismiss">' + $.t("jobs.dismiss") + "</a>"
          : "") +
        "</p>";
      ...
      $(userInfo.jobs).each(function (i, job) {
        ...
        var title =
          ...
          $.t("jobs.feeds") +
          "&lt;/b&gt; " +
          (job.feed_success + job.feed_failure) +
          "/" +
          job.feed_total;
        ...
        var progress = Math.round(
          ((job.feed_success + job.feed_failure) / job.feed_total) * 100
        );
        html +=
          '<div class="bar"><div class="label">' +
          $.t("jobs.feeds") +
          "&nbsp;</div>" +
          '<div class="job" title="' +
          title +
        ...
        var title =
          ...
          $.t("jobs.starred") +
          "&lt;/b&gt; " +
          (job.starred_success + job.starred_failure) +
          "/" +
          job.starred_total;
...
r.user.isDisplayUnread = function () {
  return r.main.mobile
    ? r.user.userInfo.display_unread_mobile
    : r.user.userInfo.display_unread_web;
};
...
r.user.isDisplayTitle = function () {
  return r.main.mobile
    ? r.user.userInfo.display_title_mobile
    : r.user.userInfo.display_title_web;
};
